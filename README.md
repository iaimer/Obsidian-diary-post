...
| **图片上传** | 支持手机/电脑上传照片，浏览器端 Canvas 压缩，可自定义压缩参数 |
| **历史日记** | 月视图日历，蓝色圆点标记有记录日期，支持图片查看 |
| **习惯统计** | 近 30 天趋势图和热力图 |
...
- 图片上传与浏览器端 Canvas 压缩（长边 2000px / 目标 < 2MB）
- 远程模式图片上传（手机端 → API Server）
- 图片压缩参数可自定义（分辨率/质量/文件大小/文件名格式）
- 今日日记图片大图查看（点击缩略图全屏弹窗）
- 日历格子优化（蓝色圆点替代数字，修复今日有图片时日期不显示）
